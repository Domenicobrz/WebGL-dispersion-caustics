...
bvhtree.BVH.prototype.intersectRay = function(rayOrigin, rayDirection, backfaceCulling) {
    ...
    for (i = 0; i < trianglesInIntersectingNodes.length; i++) {
        ...
        var intersectionRes = bvhtree.BVH.intersectRayTriangle(a, b, c, rayOriginVec3, rayDirectionVec3, backfaceCulling);

        if (intersectionRes) {
            intersectingTriangles.push({
                triangle: [a.clone(), b.clone(), c.clone()],
                triangleIndex: triIndex,
                intersectionPoint: intersectionRes.point,
                backface: intersectionRes.backface
            });
        }
    }
...
bvhtree.BVH.intersectRayTriangle = (function () {
    ...
    return function (a, b, c, rayOrigin, rayDirection, backfaceCulling) {
        ...
        return { point: result.copy( rayDirection ).multiplyScalar( t ).add( rayOrigin ), backface: sign === 1 ? true:false };
    };
}());
